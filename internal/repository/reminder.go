...
func (r *ReminderRepository) Create(ctx context.Context, reminder *models.Reminder) error {
	return r.db.Pool.QueryRow(ctx,
		`INSERT INTO reminders (user_id, enabled, recurrence_rule, messages, remind_at, description, tags)
		 VALUES ($1, $2, $3, $4, $5, $6, $7)
		 RETURNING reminders_id, created_at`,
		reminder.UserID, reminder.Enabled, reminder.RecurrenceRule, reminder.Messages,
...
func (r *ReminderRepository) GetByUserID(ctx context.Context, userID int64) ([]*models.Reminder, error) {
	rows, err := r.db.Pool.Query(ctx,
		`SELECT reminders_id, user_id, enabled, recurrence_rule, messages, remind_at, description, tags, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&reminder.ReminderID, &reminder.UserID, &reminder.Enabled, &reminder.RecurrenceRule,
			&reminder.Messages, &reminder.RemindAt, &reminder.Description, &reminder.Tags, &reminder.CreatedAt); err != nil {
			return nil, err
		}
...
func (r *ReminderRepository) GetByID(ctx context.Context, reminderID int, userID int64) (*models.Reminder, error) {
	...
	err := r.db.Pool.QueryRow(ctx,
		...
		&reminder.Messages, &reminder.RemindAt, &reminder.Description, &reminder.Tags, &reminder.CreatedAt)
	if err != nil {
		return nil, err
	}
	return reminder, nil
...
func (r *ReminderRepository) Update(ctx context.Context, reminder *models.Reminder) error {
	_, err := r.db.Pool.Exec(ctx,
		`UPDATE reminders SET enabled = $1, recurrence_rule = $2, messages = $3, remind_at = $4, description = $5, tags = $6
		 WHERE reminders_id = $7 AND user_id = $8`,
		reminder.Enabled, reminder.RecurrenceRule, reminder.Messages, reminder.RemindAt,
